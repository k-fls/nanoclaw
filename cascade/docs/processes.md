...
3. **Validator** (`intake-validate`) runs on the enriched plan inside `cascade triage` and enforces plan invariants: cache-key match, contiguity + coverage + non-overlap of commits across groups, break-point singletons, intersection-coverage attention floor, predicted-conflicts-require-resolution-flag, inspection-verdict attention floors (see [inspection.md](inspection.md) § Triage outcome mapping). On violations, triage re-invokes the agent with the failed plan and violations as input; up to `--max-retries` (default 3) before surfacing an error.
