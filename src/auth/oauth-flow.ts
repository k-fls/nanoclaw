 ...
 * events to the session interaction queue.
 *
...
/**
 * Parse an OAuth authorization URL, build an InteractionEntry, push to the
 * session's interaction queue. Returns the interactionId.
 */
function pushOAuthFlow(
  ...
  // interactionId format: providerId:callbackPort:stateHash
  ...
  try {
    ...
    const stateHash = crypto
      ...
      .digest('base64url')
      .slice(0, 8);
    interactionId = `${providerId}:${callbackPort || 0}:${stateHash}`;
  } catch (err) {
    logger.warn(
      { err, providerId, url },
      'Failed to parse OAuth URL for interactionId',
    );
    interactionId = `${providerId}:0:${Date.now()}`;
  }
...
/**
 * Wire all auth callbacks that bridge the proxy to the session interaction queues.
...
export function wireAuthCallbacks(proxy: CredentialProxy): void {
  ...
  // Authorize-stub handler pushes OAuth URLs to session's interaction queue
  ...
  // xdg-open shim pushes OAuth URLs to session's interaction queue
