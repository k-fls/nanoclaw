...
describe('SlackChannel', () => {
  ...
  describe('media handling', () => {
    it('processes file attachments via processInboundMedia', async () => {
      const opts = createTestOpts();
      const channel = new SlackChannel(opts);
      await channel.connect();
    ...
    it('skips files without download URL', async () => {
      ...
      const event = {
        ...createMessageEvent({ text: 'with broken file' }),
        files: [{ id: 'F000', name: 'broken.png', mimetype: 'image/png' }],
      };
      await triggerMessageEvent(event);
    ...
    it('skips onMessage attachments when processInboundMedia returns null', async () => {
      ...
      expect(opts.onMessage).toHaveBeenCalledWith(
        'slack:C0123456789',
        expect.objectContaining({ content: 'too large' }),
      );
      // No attachments since processInboundMedia returned null
      const call = (opts.onMessage as any).mock.calls[0][1];
      expect(call.attachments).toBeUndefined();
    });

    it('guesses media type from mimetype', async () => {
      const opts = createTestOpts();
      const channel = new SlackChannel(opts);
      await channel.connect();
