...
export class SlackChannel implements Channel {
  ...
  private setupEventHandlers(): void {
    ...
    this.app.event('message', async ({ event }) => {
      ...
      const hasFiles = !!(msg as any).files?.length;
      ...
      if (files?.length && !isBotMessage) {
        ...
        for (const file of files) {
          ...
          const result = processInboundMedia(group.folder, {
            ...
            timestamp,
            ref: { url: downloadUrl },
            caption: content !== '' ? content : undefined,
            mediaType,
          });

          if (result) {
            if (!content || content === '') content = result.content;
  ...
  async sendMedia(
    ...
    try {
      ...
      logger.info({ jid, filePath }, 'Slack media sent');
    } catch (err) {
      logger.error({ jid, filePath, err }, 'Failed to send Slack media');
    }
  }
  ...
  async downloadMedia(ref: unknown): Promise<Buffer> {
    ...
    if (!token) throw new Error('SLACK_BOT_TOKEN not available for download');
  ...
  private guessMediaType(mimetype: string): string {
