...
describe('TelegramChannel', () => {
  ...
  describe('non-text messages', () => {
    it('stores photo via processInboundMedia', async () => {
      const opts = createTestOpts();
      const channel = new TelegramChannel('test-token', opts);
      await channel.connect();
      ...
      expect(mockProcessInboundMedia).toHaveBeenCalledWith(
        'test-group',
        expect.objectContaining({
          channel: 'telegram',
          mimetype: 'image/jpeg',
          mediaType: 'image',
          ref: { fileId: 'photo_123' },
        }),
      );
      expect(opts.onMessage).toHaveBeenCalledWith(
        'tg:100200300',
    ...
    it('passes caption to processInboundMedia', async () => {
      const opts = createTestOpts();
      const channel = new TelegramChannel('test-token', opts);
      await channel.connect();
    ...
    it('stores video via processInboundMedia', async () => {
      const opts = createTestOpts();
      const channel = new TelegramChannel('test-token', opts);
      await channel.connect();
    ...
    it('stores voice via processInboundMedia', async () => {
      const opts = createTestOpts();
      const channel = new TelegramChannel('test-token', opts);
      await channel.connect();
    ...
    it('stores audio via processInboundMedia', async () => {
      const opts = createTestOpts();
      const channel = new TelegramChannel('test-token', opts);
      await channel.connect();
    ...
    it('stores document via processInboundMedia', async () => {
      const opts = createTestOpts();
      const channel = new TelegramChannel('test-token', opts);
      await channel.connect();
    ...
    it('stores sticker via processInboundMedia', async () => {
      const opts = createTestOpts();
      const channel = new TelegramChannel('test-token', opts);
      await channel.connect();
    ...
    it('stores location with placeholder (no media processing)', async () => {
      const opts = createTestOpts();
      const channel = new TelegramChannel('test-token', opts);
      await channel.connect();
      ...
      expect(mockProcessInboundMedia).not.toHaveBeenCalled();
      expect(opts.onMessage).toHaveBeenCalledWith(
        'tg:100200300',
    ...
    it('stores contact with placeholder (no media processing)', async () => {
      const opts = createTestOpts();
      const channel = new TelegramChannel('test-token', opts);
      await channel.connect();
    ...
    it('skips onMessage when processInboundMedia returns null', async () => {
      mockProcessInboundMedia.mockReturnValueOnce(null as any);
      const opts = createTestOpts();
      const channel = new TelegramChannel('test-token', opts);
      await channel.connect();
      ...
      expect(mockProcessInboundMedia).toHaveBeenCalled();
      expect(opts.onMessage).not.toHaveBeenCalled();
    });
  });
